...
@Autonomous(name="AutoWithImuLow", group="Exercises")
@Disabled
public class AutoWithImuLow extends LinearOpMode
{
    ...
    public void runOpMode() throws InterruptedException
    {
        ...
        if (opModeIsActive()) //right = outside neg
        {
            ...
            encoderStrafe(DRIVE_SPEED, -11.5, 11.5, 11.5, -11.5, 3.0);
            rotate(-getAngle(), 0.3);
            clawtime.reset();
            while(clawtime.seconds() < 1.28){
                arm_left.setPower(1);
            ...
            arm_left.setPower(0.02);
            arm_right.setPower(0.02);
            encoderDrive(DRIVE_SPEED, -10.5, -10.5, 1.0);
            rotate(-getAngle(), 0.3);
            /*
            ...
            rotate(-getAngle(), 0.3);
            //left pos
            //encoderStrafe(DRIVE_SPEED, -14, 14, 14, -14, 2.0);
            //rotate(-getAngle(), 0.3);
            //mid pos
            //encoderStrafe(DRIVE_SPEED, 14, -14, -14, 14, 2.0);
            //rotate(-getAngle(), 0.3);
            //right pos
            //encoderStrafe(DRIVE_SPEED, 38, -38, -38, 38, 4.0);
            //rotate(-getAngle(), 0.3);
    ...
    public void liftRTP(double speed,
                        int ticksL, int ticksR) {
        ...
        if (opModeIsActive()) {
            ...
            slide_left.setTargetPosition(-(int)ticksL);
            slide_right.setTargetPosition(-(int)ticksR);
            ...
            slideTimer.reset();
            // reset the timeout time and start motion.
            slide_left.setPower(Math.abs(speed));
            slide_right.setPower(Math.abs(speed));
            ...
            // onto the next step, use (isBusy() || isBusy()) in the loop test.
            while (opModeIsActive() &&
                    (slideTimer.seconds() < 2)) {

                // Display it for the driver.
                telemetry.addData("Running to",  " %7d :%7d", slide_left.getTargetPosition(),  slide_right.getTargetPosition());
                telemetry.addData("Currently at",  " at %7d :%7d",
                        slide_left.getCurrentPosition(), slide_right.getCurrentPosition());
                telemetry.update();
            }
